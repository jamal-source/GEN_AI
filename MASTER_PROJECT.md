...
| Tombol Lampirkan File           | public/index.html                | ~~Fitur palsu hanya alert()~~ **FIX v3.0.1: lampiran gambar chat nyata** (kompresi client-side → Gemini `inlineData` / catatan jujur utk Groq & OpenRouter) |
...
| Tombol Upload Foto              | public/index.html                | ~~Membuka modal yang salah~~ **FIX v3.0.1: upload foto produk aktif nyata** (canvas ≤640px, localStorage `mitraku_active_product`, thumbnail dashboard + preview modal) |
...
| ---------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 14 Sep 2026 | v3.0.1 | **UPLOAD & LAMPIRKAN SELESAI (opsional MASTER ROADMAP).** Foto produk aktif: unggah dari modal produk (kompresi client-side canvas ≤640px JPEG), thumbnail di kartu dashboard + preview modal, tersimpan di `mitraku_active_product` (guard kuota localStorage). Lampirkan gambar di chat: tombol paperclip di composer → chip preview → body `/api/chat` membawa `image` (base64) → **Gemini**: part `inlineData` (mime+base64) pada pesan user terakhir; **Groq/OpenRouter** (teks-saja): catatan jujur `[Pengguna melampirkan 1 gambar…]` disisipkan, gambar tidak dikirim (tanpa fitur palsu). Validasi: ukuran gambar ≤6MB string, jenis `data:image/` saja, body 413/400 aman. **Fix akar masalah:** hoist `GROUP_LABELS` (sebelumnya TDZ `ReferenceError` saat reload → percakapan & produk aktif hilang). QA UI `.qa_p9_upload.cjs` 15/15 + regresi p2 19/19, p7 13/13; backend 400/413/graceful terverifikasi. Cache-bust `?v=6.1`. |
