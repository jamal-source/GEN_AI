...
- **Program:** Hacktiv8 — AI Productivity & AI API Integration for Developers
...
> *"AI yang otomatis generate 9 gambar PNG (1080x1080px) + 1 video MP4 promosi untuk UMKM"*
...
### 2.1 Nama Produk
**MitraKu AI** — *"Mitra Digital Cerdas untuk UMKM Indonesia"*
...
### FASE 7 — Integrasi Langflow RAG + n8n Automation (✅ SELESAI)
- [x] Buat `langflow/mitraku_ingestion_flow.json` — Flow A: embed katalog produk ke Astra DB (Google Gemini Embeddings + AstraDB Vector Store)
- [x] Buat `langflow/mitraku_rag_chat_flow.json` — Flow B: RAG Chat CS dengan vector search + Memory session + Gemini 2.0 Flash
- [x] Update `/api/toko-pintar/chat` — arsitektur hybrid: Langflow RAG (jalur utama) → Gemini/Groq fallback otomatis
- [x] Update `POST /api/toko-pintar/catalog` — fire-and-forget trigger Langflow ingestion + notifikasi n8n saat produk ditambah
- [x] Update `DELETE /api/toko-pintar/catalog/:id` — re-ingest katalog penuh ke Astra DB + notifikasi n8n saat produk dihapus
- [x] Tambah helper `callLangflowRAG()`, `triggerLangflowIngestion()`, `notifyN8nCatalogChange()` ke `index.js`
- [x] Tambah service `n8n` ke `docker-compose.yml` (port 5678, auth admin, depends_on postgres)
- [x] Update `.env.example` — tambah `LANGFLOW_URL`, `LANGFLOW_INGESTION_FLOW_ID`, `LANGFLOW_RAG_FLOW_ID`, `ASTRA_DB_*`, `N8N_*`
...
|---|---|---|
| 9 Sep 2026 | v2.7 | **FASE 7 SELESAI.** Integrasi Langflow RAG nyata + n8n Automation: 2 Langflow Flow JSON baru (Ingestion Flow A & RAG Chat Flow B), `/api/toko-pintar/chat` sekarang hybrid Langflow→Gemini fallback, auto-trigger embedding ke Astra DB setiap katalog berubah, n8n service aktif di docker-compose, `.env.example` lengkap dengan semua variabel RAG & automation. |
...
| 9 Sep 2026 | v2.1 | **FASE 0 SELESAI.** Hapus 9 file Python deprecated, 4 n8n workflow JSON, semua scratch scripts, test scripts, output render lama, credentials Google, temp files, folder __MACOSX, starter, duplikat docker-compose & .env. Bersih dari 40+ file tidak perlu. |
